defmodule GittyWeb.DocumentChannel do
  require Logger
  use Phoenix.Channel

  alias GittyWeb.Presence

  alias Gitty.Repo
  alias Gitty.Accounts.User
  alias Gitty.Document
  alias Gitty.DocumentRegistry
  alias Gitty.DocumentSession

  Phoenix.Channel.intercept ["change"]
  ...
  def handle_out("change", payload, socket) do
      ...
      if payload[:siteId] != socket.assigns.site_id do
          Phoenix.Channel.push socket, "change", payload
      end
      {:noreply, socket}
  end
  ...
  def handle_info(:after_join, socket) do
      ...
      Phoenix.Channel.push socket, "init", %{ state: init_value, site: socket.assigns.site_id }
  ...
  def handle_presence(socket) do
      ...
      Phoenix.Channel.push socket, "presence_state", Presence.list(socket)
  end
