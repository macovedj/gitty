defmodule Gitty.DocumentSession do
  require Logger
  use Agent
  ...
  def start_link(document_id, crdt) do
      ...
      {:ok, session} = Agent.start_link(fn -> %DocumentSession{document_id: document_id,
  ...
  def save(session) do
      ...
      if last_save < last_update do
          crdt = get(session)
          GittyWeb.DocumentController.save(document_id, crdt)
